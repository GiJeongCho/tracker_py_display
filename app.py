...
def _controls_panel(backend: str) -> None:
    ...
    with st.expander("🎛️ 실시간 파라미터 조정 (적용 시 재시작 없이 즉시 반영)", expanded=True):
        ...
        b1, b2 = st.columns(2)
        if b1.button("✅ 적용", use_container_width=True, key="ctl_apply"):
        ...
        if b2.button("↺ 현재값 다시 불러오기", use_container_width=True, key="ctl_reload"):
...
with st.sidebar:
    ...
    with tab_upload:
        ...
        if up is not None and st.button("⬆️ 업로드 후 처리 시작", use_container_width=True):
    ...
    with tab_url:
        ...
        if st.button("▶️ 소스 적용", use_container_width=True) and src.strip():
            new_src = src.strip()
    ...
    if st.button("⏹️ 처리 중지 / 초기화", use_container_width=True,
    ...
    if st.button("🔄 누적 통계 초기화", use_container_width=True):
